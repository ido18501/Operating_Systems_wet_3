...
static void start_server(int num_threads, int queue_size) {
    ...
    if (pid == 0) {
        ...
        execl(SERVER_BIN, SERVER_BIN, port_str, threads_str, queue_str,
              (char *)NULL);
        _exit(1);
    }
    server_pid = pid;
...
static void check_status_ok(const char *status_line, const char *test_name) {
    if (strncmp(status_line, "HTTP/1.0 200 OK", 15) != 0) {
        char buf[256];
        snprintf(buf, sizeof(buf), "%s: bad status \"%s\"", test_name,
                 status_line);
        fail(buf);
    }
}
...
static int
get_content_length(char **headers, int nheaders, const char *test_name) {
...
static void
check_stat_headers(char **headers, int nheaders, const char *test_name) {
    const char *keys[7] = {
            "Stat-Req-Arrival::",
            "Stat-Req-Dispatch::",
            "Stat-Thread-Id::",
...
static void test_404_not_found() {
    const char *req =
            ...
            "\r\n";
    size_t rlen;
    char *resp = http_request(req, &rlen);
    if (!resp) { fail("test_404_not_found: no response"); }
    char *status = NULL, **headers = NULL;
    int nh = 0;
    const char *body;
    size_t blen;
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_404_not_found: malformed");
    }
    if (strncmp(status, "HTTP/1.0 404", 12)) {
        fail("test_404_not_found: status!=404");
    }
    check_stat_headers(headers, nh, "test_404_not_found");
    free(status);
    free_headers(headers, nh);
    free(resp);
...
static void test_403_forbidden_static() {
    ...
    if (write_file(path, "X", 0000)) {
        fail("test_403_forbidden_static: write failed");
    }
    char req[256];
    ...
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_403_forbidden_static: malformed");
    }
    if (strncmp(status, "HTTP/1.0 403", 12)) {
        fail("test_403_forbidden_static: status!=403");
    }
...
static void test_403_forbidden_dynamic() {
    ...
    if (write_file(path,
                   "#!/bin/bash\n"
                   "echo hi\n", 0644)) {
        fail("test_403_forbidden_dynamic: write failed");
    }
    chmod(path, 0644);
    char req[256];
    snprintf(req, sizeof(req),
    ...
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_403_forbidden_dynamic: malformed");
    }
    if (strncmp(status, "HTTP/1.0 403", 12)) {
        fail("test_403_forbidden_dynamic: status!=403");
    }
...
static void test_501_not_implemented() {
    ...
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_501_not_implemented: malformed");
    }
    if (strncmp(status, "HTTP/1.0 501", 12)) {
        fail("test_501_not_implemented: status!=501");
    }
...
static void test_static_get() {
    ...
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_static_get: malformed");
    }
    ...
    printf(body);
    printf("\n");
    if (memcmp(body, TEST_FILE_CONTENT, exp)) {
        fail("test_static_get: body mismatch");
    }
...
static void test_dynamic_get() {
    ...
    if (parse_response(resp, rlen, &status, &headers, &nh, &body, &blen)) {
        free(resp);
        fail("test_dynamic_get: malformed");
    }
    ...
    copy[blen] = 0;
    if (!strstr(copy, EXPECTED_CGI_MARKER)) {
        fail("test_dynamic_get: marker missing");
    }
    free(copy);
...
static void test_no_spinlock() {
    ...
    if (getrusage(RUSAGE_CHILDREN, &ru_start)) {
        fail("test_no_spinlock: rusage start");
    }
    sleep(1);
    if (getrusage(RUSAGE_CHILDREN, &ru_end)) {
        fail("test_no_spinlock: rusage end");
    }
    double utime_start =
