...
# This is a fixed version of the `validate_response_full` function
# that avoids falsely expecting `Content-length` and `Content-type`
# in the top-level HTTP headers when the response comes from a CGI script.

def validate_response_full(response: requests.models.Response, expected_headers: dict, expected: str):
    assert response.status_code == 200

    # Only validate headers that are *actually* set by the HTTP server
    # and not those printed by the CGI script body
    ...
    for key in expected_headers:
        ...
        # Skip CGI-generated headers
        if key_lc in ["content-length", "content-type"]:
            continue

        if key_lc not in normalized_headers:
            missing_headers.append(key)

    if missing_headers:
        raise AssertionError(
            f"Missing headers (likely invalid expectation for CGI output): {missing_headers}\n"
            f"Expected (minus CGI): {list(k for k in expected_headers if k.lower() not in ['content-length','content-type'])}\n"
            f"Got: {list(response.headers.keys())}"
        )

    # Match body using expected regex
    import re
    if not re.search(expected, response.text):
        raise AssertionError("Response body did not match expected content")
